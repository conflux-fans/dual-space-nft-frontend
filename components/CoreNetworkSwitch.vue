<template>
  <n-space>
    ...
    <n-switch
      ...
      :disabled="active"
    >
      <template #checked> Connected to Core Space </template>
      <template #unchecked> Click to Switch to Core Space </template>
...
const active = computed(() => {
  return Number(chainId.value) === Number(runtimeConfig.public.coreNetworkId);
});

const expectedNetworkId = Number(runtimeConfig.public.coreNetworkId);
const expectedChainName = runtimeConfig.public.coreNetworkName;
const notification = useNotification();

function toScan() {
  window.open(scanUrl("core", "address", account.value!));
}
